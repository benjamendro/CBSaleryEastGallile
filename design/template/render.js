...
(function(){
  ...
  const CHART_KINDS=['barColumns','barRanked','grouped','donut','line','multiLine'];
  const pending=[]; // {id, fig, spec}

  /* ---- tabbed multi-chart section ---- */
  build.tabbed = function(s){
    const groupId='tg-'+(chartSeq++);
    const btnHtml=s.tabs.map((t,i)=>
      `<button class="tab-btn${i===0?' active':''}" onclick="RC.switchTab('${groupId}',${i})">${t.label}</button>`
    ).join('');
    const panelsHtml=s.tabs.map((t,i)=>{
      const ts=Object.assign({},t);
      if(ts.series&&!ts.legend) ts.legend=legendFromSeries(ts.series);
      const f=figure(ts);
      pending.push(f);
      const anim=i===0?' data-animated="1"':'';
      return `<div class="tab-panel${i===0?'':' hidden'}"${anim}><div style="max-width:920px;margin:0 auto">${f.fig}</div></div>`;
    }).join('');
    return `<div class="wrap">
      ${(s.kicker||s.heading)?`<div class="measure" style="margin-bottom:28px">${kicker(s.kicker)}${s.heading?`<h2 class="reveal">${s.heading}</h2>`:''}</div>`:''}
      <div class="tab-bar" id="${groupId}">${btnHtml}</div>
      <div class="tab-panels" data-group="${groupId}">${panelsHtml}</div>
    </div>`;
  };
